...
- (void) backToMain
{
    ...
    // Pop to main view
    DefragAppDelegate *delegate = (DefragAppDelegate *) [[UIApplication sharedApplication] delegate];
    DefragViewController *defragViewController = (DefragViewController *) [[self.navigationController viewControllers] objectAtIndex:0];
    // Set the delegate to the main root view controller
    // to ensure if an SSO request comes in this controller
    // will handle it.
    [delegate facebook].sessionDelegate = defragViewController;
    [[delegate navigationController] popToDefragViewControllerAnimated:YES];
}
